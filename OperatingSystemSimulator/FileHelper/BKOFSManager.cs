...
public class BKOFSManager
{
    ...
    public void LoadFromJson()
    {
        if (File.Exists(JsonFilePath))
        {
            ...
            if (data != null)
            {
                ...
                ResetAllFileLocks(RootDirectory);
            }
            else
            {
                Initialize();
    ...
    private void ResetAllFileLocks(BKOFSDirectory directory)
    {
    ...
    private static async Task<bool> ChangeFileInternal(int fileID, BKOFSDirectory parentDirectory, string? newName, bool? isRestricted, string? newContent, bool? needsPrinting = true)
    {
        ...
        if (file.IsBusy)
        {
            ...
            if (needsPrinting == true)
            {
                ConsoleLogger.Log($"Couldn't change the file {file.Name}, Reason: File is currently busy (locked).", LogType.Error);
                MessageManager.Instance.CreateMessage(1, "File In Use", $"Couldn't change the file {file.Name} because it is currently in use by another process.", ShellType.None);
            }
            return false;
        }
    ...
    private static async Task<bool> DeleteFileInternal(int fileID, BKOFSDirectory parentDirectory)
    {
        ...
        {
            await ResetHDStatus(50);
            ConsoleLogger.Log($"Couldn't delete the file {file.Name}, Reason: File is currently busy (locked).", LogType.Error);
            MessageManager.Instance.CreateMessage(1, "File In Use", "You cannot delete file " + file.Name + " because it is currently in use!", ShellType.None);
            return false;
        }
