...
public class HardwarePageViewModel : INotifyPropertyChanged
{
    ...
    public void IndicateContextSwitch(string lastProcess, string currentProcess)
    {
        if (hardwarePage != null)
        {
            hardwarePage.DispatcherQueue.TryEnqueue(() => 
            {
                hardwarePage.ShowContextSwitch(lastProcess, currentProcess);
            });
        }
    }
